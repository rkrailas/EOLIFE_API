...
if ($received_data->action == "AddSales") {
    ...
    try {
        ...
        if ($have_error) {
            ...
            $gldescription = "";
            if ($received_data->data->soreturn == "N") {
                $gldescription = "ขายสินค้า-ลูกค้า:" . $received_data->data->customer->name . "-ใบสั่งขาย:" . $received_data->data->snumber;
            }elseif ($received_data->data->soreturn == "V") {
                $gldescription = "ให้บริการ-ลูกค้า:" . $received_data->data->customer->name . "-ใบแจ้งหนี้:" . $received_data->data->snumber;
            }
            ...
            $data = [
                ...
                'exclusivetax' => '1',
                'closed' => '1',
                'taxrate' => $received_data->data->taxratestd,
                'taxratestd' => $received_data->data->taxratestd,
                'salesaccount' => $salesaccount,
                'soreturn' => $received_data->data->soreturn,
            ...
            $strsql = "INSERT INTO sales(customerid,snumber,sonumber,sodate,journaldate,deliverydate,expirydate,duedate,sodiscount,sototal,salestax
                        ,exclusivetax,closed,taxrate,taxratestd,salesaccount,soreturn,gldescription,employee_id,transactiondate)
            ...
            foreach ($received_data->data->salesdetail as $row) {
                ...
                if ($have_error) {
                    ...
                    $strsql = "INSERT INTO salesdetail(snumber,sdate,itemid,description,quantity,quantityord,quantitybac,unitprice,discountamount,amount,taxamount,taxrate,salesac,stocktype,soreturn,employee_id,transactiondate)
                                VALUES(:snumber,:sdate,:itemid,:description,:quantity,:quantityord,:quantitybac,:unitprice,:discountamount,:amount,:taxamount,:taxrate,:salesac,:stocktype,:soreturn,:employee_id,:transactiondate)";
                    $stmt = $conn->prepare($strsql);
                    $stmt->execute($data);
